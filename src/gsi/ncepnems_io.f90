...
!$$$ end documentation block
    ...
    if (mype==0) write(6,*)'npe num_fields = ', npe,n3du*grd_a%nsig+n2d,num_fields
...
!      Cloud Amount (cloud fraction) - should be the same as the input guess values
       do k=1,grd%nsig
          if (mype==mype_out) then
             ...
             call nemsio_readrecv(gfile,'cnvw','mid layer',k,rwork1d,iret=iret)
             if (iret == 0) then
                call nemsio_writerecv(gfileo,'cnvw','mid layer',k,rwork1d,iret=iret)
                if (iret /= 0) call error_msg(trim(my_name),trim(filename),'cnvw','write',istop,iret)
             endif
             call nemsio_readrecv(gfile,'cnvc','mid layer',k,rwork1d,iret=iret)
             if (iret == 0) then
                call nemsio_writerecv(gfileo,'cnvc','mid layer',k,rwork1d,iret=iret)
