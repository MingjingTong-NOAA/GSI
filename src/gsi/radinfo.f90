...
! set passed variables to public
  ...
  public :: crtm_overlap,rewopt,reiopt,crtm_gfdl,allsky_verbose
...
!                                                          certain instruments. Set to greater than zero to use  
  ...
  logical :: crtm_gfdl,allsky_verbose
...
!$$$ end documentation block
    ...
    crtm_gfdl = .true.    ! .true.= use gfdl crtm configuration
...
!      Initialize predx if inew_rad and compute angle bias correction and tlapmean
       if (adp_anglebc) then
          !write(507,*)'RADINFO_READ: new radinfo '
          call init_predx
          cbias=zero
          do j=1,jpch_rad
...
!         check inew_rad again
          do j =1,jpch_rad
            ...
            !if (mype==mype_rad) write(507,111) j,nusis(j), &
            !nuchan(j),varch(j),varch_cld(j),iuse_rad(j),ermax_rad(j), &
            !b_rad(j),pg_rad(j),iextra_det(j),icloud4crtm(j),iaerosol4crtm(j)
          end do
       end if
