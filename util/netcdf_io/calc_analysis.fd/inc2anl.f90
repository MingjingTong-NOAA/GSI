...
contains
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    use vars_calc_analysis, only: mype, cliptracers
    implicit none
    ! variables local to this subroutine
    integer :: i, j, iincvar
    logical :: use_increment, cliptracer
    real :: clip
    ...
    do i=1,nnciov
      ...
      if (use_increment) then
        if (iovars_netcdf(i) == 'pressfc') then
          ...
          if (iovars_netcdf(i) == 'spfh   ' .or. iovars_netcdf(i) == 'clwmr  ' .or. &
              iovars_netcdf(i) == 'icmr   ' .or. iovars_netcdf(i) == 'rwmr   ' .or. & 
              iovars_netcdf(i) == 'snmr   ' .or. iovars_netcdf(i) == 'grle   ' .or. &
              iovars_netcdf(i) == 'o3mr   ') then
             cliptracer = cliptracers
          else
             cliptracer = .false.
          end if
          ! call generic subroutine for all other fields
          if (mype==0) print *, 'Adding Increment to ', iovars_netcdf(i), &
                                 incvars_netcdf(iincvar), 'cliptracer ', cliptracer
          if (trim(iovars_netcdf(i)) == 'spfh' .or. trim(iovars_netcdf(i)) == 'o3mr') then
             clip=tiny(0.0)
          else
             clip=0.0
          end if  
  ...
  subroutine add_increment(fcstvar, incvar, cliptracer, clip)
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do k=1,nlev
        if (mype == levpe(k)) then
          ...
          if (has_var(incncfile, trim(incvar)//"_inc")) then
            ...
            if (mype == 0) write(6,*) trim(incvar)//"_inc", ' not in increment file, skipping...'
          end if
        end if
      end do
      ! clean up and close
